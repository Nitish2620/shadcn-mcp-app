...
/* ========================================================
   SHOP AVATAR DECORATIONS FRAME RENDERER (BOUNDED LAYOUT)
======================================================== */
const AvatarDecorationFrame = React.memo(({ decoration, isUnlocked }: { decoration?: AvatarDecoration; isUnlocked: boolean }) => {
  ...
  if (decoration === 'anime_power_aura') {
    return (
      <div className="absolute inset-0 rounded-full z-10 pointer-events-none transform scale-125">
        <motion.div 
          animate={{ rotate: 360, scale: [1, 1.05, 1] }}
          transition={{ repeat: Infinity, duration: 4, ease: 'linear' }}
          className="w-full h-full rounded-full border-2 border-amber-400/90 shadow-[0_0_12px_rgba(251,191,36,0.8)] opacity-90"
        />
        <div className="absolute top-0 right-1 w-2 h-2 bg-amber-300 rounded-full blur-xs animate-ping" />
      </div>
    );
  }
  ...
  if (decoration === 'cyber_hacker_void') {
    return (
      <div className="absolute inset-0 rounded-full z-10 pointer-events-none transform scale-125">
        <div className="w-full h-full rounded-full border-2 border-emerald-400 animate-pulse shadow-[0_0_12px_rgba(52,211,153,0.8)]" />
  ...
  if (decoration === 'celestial_orbit') {
    return (
      <motion.div 
        ...
        className="absolute inset-0 rounded-full border-2 border-dashed border-indigo-400 z-10 pointer-events-none transform scale-125 shadow-[0_0_12px_rgba(129,140,248,0.8)]"
      >
        <div className="absolute -top-1 left-1/2 w-2.5 h-2.5 bg-indigo-300 rounded-full shadow-lg" />
      </motion.div>
    );
  }
...
export function ChatListCard({
  ...
  return (
    <Tooltip.Provider>
      <div className="w-full max-w-6xl mx-auto font-sans selection:bg-indigo-500 selection:text-white relative">
        ...
        <div className="flex items-center justify-between gap-3 mb-3 px-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold bg-[#5865f2] hover:bg-[#4752c4] text-white shadow-md transition transform hover:scale-105 cursor-pointer"
          >
        ...
        <div className="bg-white/95 dark:bg-[#313338]/95 border border-slate-200/80 dark:border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row h-[720px] backdrop-blur-xl relative">
          ...
          <div className="w-full md:w-80 border-r border-slate-200/80 dark:border-slate-800/80 flex flex-col bg-slate-50/70 dark:bg-[#2b2d31]/80 shrink-0">
            ...
            <div className="p-4 border-b border-slate-200/60 dark:border-slate-800/60 flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-[#1e1f22] pl-9 pr-3 py-2 rounded-xl text-xs border border-slate-200 dark:border-slate-800 outline-none focus:ring-2 focus:ring-[#5865f2] font-medium text-slate-900 dark:text-white"
                />
              </div>
              <button
                type="button"
                ...
                className="p-2 rounded-xl bg-[#5865f2] hover:bg-[#4752c4] text-white font-bold text-xs flex items-center justify-center cursor-pointer transition shadow-xs shrink-0"
                title="New Chat DM"
              >
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 rounded-2xl cursor-pointer transition flex items-center gap-3 relative ${
                        isSelected 
                          ? 'bg-[#5865f2]/15 dark:bg-[#5865f2]/25 border border-[#5865f2]/40 shadow-xs' 
                    ...
                    >
                      ...
                      <div className="relative shrink-0 w-11 h-11 flex items-center justify-center">
                        {isChannel ? (
                          <div className="w-full h-full rounded-2xl bg-[#5865f2]/20 border border-[#5865f2]/40 flex items-center justify-center text-[#5865f2] font-bold text-base">
                          ...
                          <>
                            ...
                            />
                            {chat.isOnline && (
                              <span className="w-3 h-3 bg-emerald-500 rounded-full border-2 border-white dark:border-[#2b2d31] absolute bottom-0 right-0 z-20" />
                            )}
                          </>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {chat.isTyping ? (
                          <span className="text-[10px] text-[#5865f2] font-bold flex items-center gap-1 animate-pulse">
                            <span>typing</span>
                            <span className="w-1 h-1 bg-[#5865f2] rounded-full animate-ping" />
                          </span>
                        ) : (
                      ...
                      {chat.unreadCount && chat.unreadCount > 0 && (
                        <span className="bg-[#5865f2] text-white font-extrabold text-[10px] px-2 py-0.5 rounded-full shrink-0">
                          {chat.unreadCount}
                        </span>
                      )}
                    </div>
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-[#313338]">
            ...
            <div className="p-4 border-b border-slate-200/80 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-[#2b2d31]/50">
              ...
              <div className="flex items-center gap-2">
                        ...
                        {pinnedMessages.length === 0 ? (
                          ...
                          pinnedMessages.map(pm => (
                            <div key={pm.id} className="p-2 bg-slate-800/80 rounded-xl border border-slate-700/60 text-xs">
                              <div className="font-bold text-[#5865f2] text-[10px]">{pm.senderName} • {pm.timestamp}</div>
                ...
                <button type="button" onClick={() => setSoundEnabled(!soundEnabled)} className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 cursor-pointer">
                  {soundEnabled ? <Volume2 className="w-4 h-4 text-[#5865f2]" /> : <VolumeX className="w-4 h-4 text-slate-400" />}
                </button>
              </div>
            </div>
            ...
            {/* Messages Feed Stream without Horizontal Scrollbars */}
              ...
              {activeChat.messages.map((msg) => {
                return (
                  <div key={msg.id} className={`flex items-start gap-3 group relative ${msg.isMe ? 'flex-row-reverse' : ''}`}>
                    ...
                    <Popover.Root>
                      <Popover.Trigger asChild>
                        <button type="button" className="relative w-9 h-9 shrink-0 cursor-pointer flex items-center justify-center">
                          <AvatarDecorationFrame decoration={msg.avatarDecoration} isUnlocked={isNitroPro} />
                          <img src={isNitroPro && msg.animatedAvatar ? msg.animatedAvatar : msg.senderAvatar} alt={msg.senderName} className="w-full h-full rounded-full object-cover relative z-10 hover:ring-2 hover:ring-[#5865f2] transition" />
                        </button>
                      </Popover.Trigger>
                      <Popover.Portal>
                        <Popover.Content className="bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-2xl z-50 w-60 text-white space-y-3">
                          <div className="flex items-center gap-3">
                            <img src={msg.senderAvatar} alt={msg.senderName} className="w-12 h-12 rounded-full object-cover ring-2 ring-[#5865f2]" />
                            <div>
                              <div className="font-bold text-xs">{msg.senderName}</div>
                              <div className="text-[10px] text-[#5865f2] font-mono">@member</div>
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            className="w-full py-1.5 rounded-xl bg-[#5865f2] hover:bg-[#4752c4] text-white text-xs font-bold transition cursor-pointer"
                          >
                            Send Direct Message
                          </button>
                        </Popover.Content>
                    ...
                    <div className={`max-w-md space-y-1 ${msg.isMe ? 'items-end text-right' : ''}`}>
                      ...
                      {msg.replyToMessage && (
                        <div className="text-[10px] bg-slate-200/80 dark:bg-slate-800/80 border-l-2 border-[#5865f2] px-2.5 py-1 rounded-r-lg text-slate-600 dark:text-slate-300 font-medium">
                          Replying to <span className="font-bold text-[#5865f2]">{msg.replyToMessage.senderName}</span>: "{msg.replyToMessage.text}"
                        </div>
                      )}
                      ...
                      <div className={`p-3.5 rounded-2xl text-xs leading-relaxed border relative shadow-xs ${
                        msg.isMe 
                          ...
                          : 'bg-slate-100 dark:bg-[#2b2d31] text-slate-900 dark:text-slate-100 border-slate-200/80 dark:border-slate-700/60'
                      }`}>
                        <p>{msg.text}</p>
                    ...
                    <div className="hidden group-hover:flex items-center gap-1 bg-slate-900 text-white p-1 rounded-xl shadow-lg border border-slate-800 text-[10px] absolute -top-3 right-4 z-20">
                      ...
                      <button
                        ...
                        className="p-1 hover:bg-slate-800 rounded text-[#5865f2] cursor-pointer"
            ...
            {replyTarget && (
              <div className="bg-[#5865f2]/20 border-t border-[#5865f2]/30 px-4 py-1.5 flex items-center justify-between text-xs text-[#5865f2] dark:text-purple-300">
            ...
            <div className="p-4 border-t border-slate-200/80 dark:border-slate-800 bg-slate-50/50 dark:bg-[#2b2d31]/50 space-y-2">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="flex items-center gap-2">
                  ...
                  <Popover.Portal>
                    <Popover.Content className="bg-slate-900 border border-slate-800 p-3 rounded-2xl shadow-2xl z-50 w-64 space-y-2">
                      <div className="text-xs font-bold text-white flex items-center justify-between">
                        ...
                        <span className="text-[9px] font-mono text-[#5865f2]">{isNitroBasic ? 'UNLOCKED' : 'LOCKED'}</span>
                      </div>
                ...
                <input
                  ...
                  className="flex-1 bg-slate-100 dark:bg-[#1e1f22] px-4 py-2.5 rounded-xl text-xs border border-slate-200 dark:border-slate-800 outline-none focus:ring-2 focus:ring-[#5865f2] font-medium text-slate-900 dark:text-white"
                />

                <button
                  type="submit"
                  className="px-4 py-2.5 rounded-xl bg-[#5865f2] hover:bg-[#4752c4] text-white font-bold text-xs flex items-center gap-1.5 shadow-md transition cursor-pointer"
                >
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-4 rounded-2xl border text-left flex items-center justify-between cursor-pointer transition ${
                        isSelected 
                          ? 'border-[#5865f2] bg-[#5865f2]/10 dark:bg-[#5865f2]/20 shadow-md' 
                    ...
                    >
                      ...
                      <span className="font-black text-xs text-[#5865f2]">{plan.price}</span>
                    </button>
                  );
                })}
              </div>
